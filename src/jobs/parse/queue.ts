...
// Common lock release payload to DRY updates
const releaseLock = {
...
async function getJobOrThrow(id: string) {
    const job = await prisma.parseJob.findUnique({ where: { id } });
...
export async function handleFailure(jobId: string, error: Error): Promise<void> {
    ...
    if (shouldRetry) {
        ...
        const delayMs = Math.min(1000 * 2 ** job.retryCount, MAX_BACKOFF_MS);
        jobLogger.warn({
            event: "job.retry",
            jobId,
